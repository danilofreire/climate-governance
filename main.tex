...
Self-enforcing environmental governance faces further difficulties in new democracies. In Latin America, authoritarian regimes discouraged civil participation and reduced social capital, two requirements for effective polycentric arrangements \citep{aligica2014institutional,ostrom1990governing}. Top-down climate policies also create incentives for organised groups to collude with state officials and pursue private interests. In that scenario, elites and state representatives have \textit{de facto} veto power over environmental legislation and can override local decisions to their own benefit.
...
We conducted our experiment in 10 Latin American countries. Latin America is the most biodiverse region in the world and it remains largely dependent on agriculture, thus global warming can severely disrupt the regional environment and economy \citep{jones2003potential, yang2018post}. Moreover, survey experiments on climate policies have focused on a small set of developed countries, so truly comparative experimental work is still scarce. We fill this gap by including countries in South, Central, and North America where local elites have different stakes at climate change negotiations.

The hypothetical climate change agreements includes six dimensions: 1) which organisation defines the rules; 2) how would conflicts be resolved; 3) what punishment should be applied to rule-breakers; 4) how should repeated violations be sanctioned; 5) which countries should bear the costs of the agreement; 6) how often should the agreement be renegotiated. The categories map conceptual differences between polycentric and state-led climate mitigation policies \citep{bechtel2013mass, ostrom1990governing}. Table~\ref{tab:categories} describes the values we included in each design principle. \\

\begin{longtable}{l|l}
\caption{Design principles for climate change mitigation conjoint experiments} \label{tab:categories} \\

\hline \multicolumn{1}{c|}{\textbf{Attribute}} & \multicolumn{1}{c}{\textbf{Values}}\\ \hline 
\endfirsthead 

\multicolumn{2}{c}%
{{\tablename \thetable{} Design principles for climate change mitigation conjoint experiments (\textit{cont.})}} \\
\hline \multicolumn{1}{c|}{\textbf{Attribute}} & \multicolumn{1}{c}{\textbf{Values}} \\ \hline 
\endhead
